...
class Student:
	def __init__(self, root):
		...
		#First image section======	
		img=Image.open(r"collage imgs\student.jpg")
    ...
    #second image section======
		img1=Image.open(r"collage imgs\facialrecognition.png")
    ...
    #3rd image section=======
		img2=Image.open(r"collage imgs\clg.jpg")
    ...
    #background image=======
		img3=Image.open(r"collage imgs\background.jpg")
		...
		img_left=Image.open(r"collage imgs\stdnt.jpg")
		...
		dep_combo["values"]=("Select Department","CSE","IT","ME","EE","EC","CE","EEE")
		dep_combo.current(0)
		...
		course_combo["values"]=("Select Course","B.TECH","B.E.","M.TECH","M.E.")
		...
		student_sec_label=Label(Class_student_frame,text="Class Section:",font=("times new roman",12,"bold"),bg="white")
		student_sec_label.grid(row=1,column=0,padx=10,pady=5,sticky=W)		

		sec_combo=ttk.Combobox(Class_student_frame,textvariable=self.var_sec,font=("times new roman",12,"bold"),state="readonly", width=18)
		sec_combo["values"]=("1","2","3","4")
		sec_combo.current(0)
		sec_combo.grid(row=1,column=1,padx=10,pady=5,sticky=W)

		#Univ roll label
		univ_roll_label=Label(Class_student_frame,text="University roll:",font=("times new roman",12,"bold"),bg="white")
		univ_roll_label.grid(row=1,column=2,padx=10,pady=5,sticky=W)

		univ_roll_entry=ttk.Entry(Class_student_frame,  textvariable=self.var_univ_roll,width=20,font=("times new roman",12,"bold"))
		univ_roll_entry.grid(row=1,column=3,padx=10,pady=5,sticky=W)
		...
		#coordinatorlabel
		coordinator_label=Label(Class_student_frame,text="Coordinator Name :",font=("times new roman",12,"bold"),bg="white")
		coordinator_label.grid(row=4,column=2,padx=10,pady=5,sticky=W)

		coordinator_entry=ttk.Entry(Class_student_frame, textvariable=self.var_coordinator,width=20,font=("times new roman",12,"bold"))
		...
		#image section
		img_right=Image.open(r"collage imgs\group cls 02.jpg")
		...
		search_combo["values"]=("Select","univroll","id")
		...
		self.student_table=ttk.Treeview(table_frame,column=("dep","course","year","sem","id","name","sec","univ_roll","gender","dob","email","phone","address","coordinator","photo"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
		...
		self.student_table.heading("univ_roll",text="Univrisity roll")
		...
		self.student_table.column("univ_roll",width=100)
	...
	def get_cursor(self,event=""):
		...
		self.var_univ_roll.set(data[7]),
		self.var_gender.set(data[8]),
		self.var_dob.set(data[9]),
		self.var_email.set(data[10]),
		self.var_phone.set(data[11]),
		self.var_address.set(data[12]),
		self.var_coordinator.set(data[13]),
		...
		if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()=="":
			...
			try:
				...
				if Update > 0:
					conn=mysql.connector.connect(host="localhost",username="root",password="1234",database="face_recognizer")
					my_cursor=conn.cursor()
					my_cursor.execute("update student set Dep=%s,course=%s,Year=%s,Semester=%s,Name=%s,Section=%s,Univ_roll=%s,Gender=%s,Dob=%s,Email=%s,Phone=%s,Address=%s,Coordinator=%s,PhotoSample=%s where Student_id=%s",(
																														self.var_dep.get(),
																														self.var_course.get(),																																														
																														self.var_year.get(),
																														self.var_semester.get(),
